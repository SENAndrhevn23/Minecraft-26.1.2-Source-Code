   ...
   public static record Bounds<T extends Number & Comparable<T>>(Optional<T> min, Optional<T> max) {
      ...
      static <B extends ByteBuf, T extends Number & Comparable<T>> StreamCodec<B, Bounds<T>> createStreamCodec(final StreamCodec<B, T> numberCodec) {
         return new StreamCodec<B, Bounds<T>>() {
            ...
            // $FF: synthetic field
            final StreamCodec val$numberCodec;

            {
               this.val$numberCodec = val$numberCodec;
            }

            public MinMaxBounds.Bounds<T> decode(final B input) {
               byte flags = input.readByte();
               Optional<T> min = (flags & 1) != 0 ? Optional.of((Number)this.val$numberCodec.decode(input)) : Optional.empty();
               Optional<T> max = (flags & 2) != 0 ? Optional.of((Number)this.val$numberCodec.decode(input)) : Optional.empty();
               return new MinMaxBounds.Bounds<T>(min, max);
            }
            ...
            // $FF: synthetic method
            private static void lambda$encode$1(StreamCodec numberCodec, ByteBuf output, Number v) {
               numberCodec.encode(output, v);
            }

            // $FF: synthetic method
            private static void lambda$encode$0(StreamCodec numberCodec, ByteBuf output, Number v) {
               numberCodec.encode(output, v);
            }
         };
      }
