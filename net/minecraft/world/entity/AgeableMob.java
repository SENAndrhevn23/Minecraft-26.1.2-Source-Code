   ...
   public static int makeAgeLockedParticle(final Level level, final Mob mob, int ageLockParticleTimer, final boolean isAgeLocked) {
      if (ageLockParticleTimer > 0) {
         if (level.isClientSide() && ageLockParticleTimer % 2 == 0) {
            ...
            Vec3 spawnPosition = new Vec3(mob.getRandomX((double)1.0F), mob.getRandomY(0.2) + (double)mob.getBbHeight() + (double)yParticleOffset, mob.getRandomZ((double)1.0F));
            level.addParticle(isAgeLocked ? ParticleTypes.PAUSE_MOB_GROWTH : ParticleTypes.RESET_MOB_GROWTH, spawnPosition.x, spawnPosition.y, spawnPosition.z, (double)0.0F, (double)0.0F, (double)0.0F);
         }
