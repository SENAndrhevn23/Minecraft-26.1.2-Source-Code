...
public class InputControlHandlers {
   ...
   private static class TextInputHandler implements InputControlHandler<TextInput> {
      public void addControl(final TextInput input, final Screen screen, final InputControlHandler.Output output) {
         ...
         output.accept(wrappedControl, new Action.ValueGetter() {
            // $FF: synthetic field
            final Supplier val$getter;

            {
               this.val$getter = val$getter;
               Objects.requireNonNull(this$0);
               super();
            }

            public String asTemplateSubstitution() {
               return StringTag.escapeWithoutQuotes((String)this.val$getter.get());
            }

            public Tag asTag() {
               return StringTag.valueOf((String)this.val$getter.get());
            }
         });
      }
   }
   ...
   private static class BooleanHandler implements InputControlHandler<BooleanInput> {
      public void addControl(final BooleanInput input, final Screen screen, final InputControlHandler.Output output) {
         Font font = screen.getFont();
         Checkbox control = Checkbox.builder(input.label(), font).selected(input.initial()).build();
         output.accept(control, new Action.ValueGetter() {
            // $FF: synthetic field
            final Checkbox val$control;
            // $FF: synthetic field
            final BooleanInput val$input;
            ...
            public String asTemplateSubstitution() {
               return this.val$control.selected() ? this.val$input.onTrue() : this.val$input.onFalse();
            }

            public Tag asTag() {
               return ByteTag.valueOf(this.val$control.selected());
            }
         });
      }
   }
   ...
   private static class NumberRangeHandler implements InputControlHandler<NumberRangeInput> {
      public void addControl(final NumberRangeInput input, final Screen screen, final InputControlHandler.Output output) {
         ...
         SliderImpl control = new SliderImpl(input, (double)initialValue);
         output.accept(control, new Action.ValueGetter() {
            // $FF: synthetic field
            final InputControlHandlers.NumberRangeHandler.SliderImpl val$control;
            ...
            public String asTemplateSubstitution() {
               return this.val$control.stringValueToSend();
            }

            public Tag asTag() {
               return FloatTag.valueOf(this.val$control.floatValueToSend());
            }
         });
      }
