...
public class EndermiteModel extends EntityModel<EntityRenderState> {
   ...
   public void setupAnim(final EntityRenderState state) {
      ...
      for(int i = 0; i < this.bodyParts.length; ++i) {
         this.bodyParts[i].yRot = Mth.cos((double)(state.ageInTicks * 0.9F + (float)i * 0.15F * (float)Math.PI)) * (float)Math.PI * 0.01F * (float)(1 + Math.abs(i - 2));
         this.bodyParts[i].x = Mth.sin((double)(state.ageInTicks * 0.9F + (float)i * 0.15F * (float)Math.PI)) * (float)Math.PI * 0.1F * (float)Math.abs(i - 2);
      }
